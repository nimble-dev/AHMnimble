...
# "model3.txt" in the book code
Section8p5p3_code <- nimbleCode({ 
    # Priors
    alpha0 ~ dunif(-10,10)
    alpha1 ~ dunif(-10,10)
    beta0 ~ dunif(-10,10)
...
out3 <- nimbleMCMC(code = Section8p5p3_code, 
                   constants = win.data, 
                   inits = inits,
                   monitors = params,
                   nburnin = nb, 
...
colnames(out3)
mcmcplot(out3)
## samples appear to be reasonably well mixed from a visual inspection of the traceplot and autocorrelations

Section8p5p3_compare <- compareMCMCs(
  modelInfo = list(
    code = Section8p5p3_code,
    data = win.data,
    inits = inits(),
    monitors = params
  ),
...
make_MCMC_comparison_pages(Section8p5p3_compare, modelNames = "Section8p5p3", dir = outputDirectory)

browseURL(file.path(outputDirectory, "Section8p5p3.html"))
