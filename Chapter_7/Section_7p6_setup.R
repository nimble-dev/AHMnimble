...
apply(ovendata.list$data,2,sum)  # Removals in occasion 1
...
inits <- function(){
  list (p0 = runif(1), 
        alpha1 = runif(1), 
        beta0 = runif(1),
        beta1 = runif(1), # nimble's default samplers have a hard time
        beta2 = runif(1), # recovering from poor initial values,
        beta3 = runif(1), # so, some additional initial vaules were included
        N = ncap+2)       # here relative to what was in the original book code.
}
