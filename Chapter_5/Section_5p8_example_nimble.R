...
# This section can be used as an example of an advantageous feature of nimble: ability for user to define models not built-in to package  
# Avoids necessity of work-arounds such as the 0s or 1s trick shown here
...
# Code for file "multiple_linear_regression_model.txt" from original AHM code.
Section5p8_code <- nimbleCode ( {
...
# Initial values
inits <- function() list(alpha0 = rnorm(3, 0, 10), alpha1 = rnorm(3,0,10), alpha2 = rnorm(3,0,10), alpha3 = rnorm(3,0,10), sd = rep(1, 3))

# Parameters monitored (i.e., for which estimates are saved)
params <- c("alpha0", "alpha1", "alpha2", "alpha3", "sd")

# MCMC settings
ni <- 1200   ;   nt <- 1   ;   nb <- 200   ;  nc <- 3    # ((Textbooks says that JAGS is required because BUGS needs ))

# Build, configure and compile model and mcmc:
outX <- nimbleMCMC(code = Section5p8_code,
                   constants = win.data,
                   monitors = params,
                   inits = inits,
                   nburnin = nb,
...
Section5p8_compare <- compareMCMCs(
  modelInfo = list(
   code = Section5p8_code,
   data = win.data,
   inits = inits()
  ),
  MCMCs = c('nimble', 'jags'),
...
make_MCMC_comparison_pages(Section5p8_compare, modelNames = "Section5p8", dir = outputDirectory)

browseURL(Sys.glob(file.path(outputDirectory, "Section5p8.html")))
